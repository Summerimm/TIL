...
- **HTTP URI**를 통해 자원을 명시하고 **HTTP Method**를 통해 해당 자원에 대한 상태를 주고 받는 방식을 말함

# Day 10. 230802
## 1. Vue.js와 React.js의 차이
- Vue는 프레임워크이고 React는 라이브러리

## 2. 프레임워크와 라이브러리의 차이
- 프레임워크는 **소프트웨어의 구조를 설계해둔 것**이고 라이브러리는 **기능을 구현해둔 것**
- 프레임워크는 구조를 설계한 것이기 때문에 개발자가 *프레임워크의 구조에 맞게 개발*해야 함
- 라이브러리는 기능을 구현해둔 것이기 때문에 개발자는 라이브러리를 활용하여 *자유롭게 개발*할 수 있음

# Day 11. 230829
## 1. BE 개발자로 취업하기 위해서
- Java에 대한 버전 작성 시, 이전 버전과의 차이점에 대한 질문이 나올 수 있음
- JVM, GC, Java의 Collection, Spring의 MVC 구조, 디자인 패턴, 객체 지향에 대한 질문
- Spring 프레임워크의 경우, 공식 문서를 기반으로 공부하는 것이 좋음.
- 토비의 스프링, 백기선님의 강의
- DI(의존성 주입), IoC(제어의 역전), AOP(관점 지향 개발), JPA 공부
- MySQL 같은 관계형 DB
- MongoDB 같은 NoSQL
- Redis 같은 캐시
- SNS 기능 &rarr; NoSQL
- JWT 같은 로그인 인증 시 Redis 활용
- 쓰레드와 프로세스의 차이는?
- 배열과 링크드 리스트의 차이는?
- HashMap과 HashTable의 차이는?
- 평소에 사용했던 기술 스택과 관련된 지식에 대해서 깊게 생각하고 문서화 할 것 &rarr; 깊은 이해가 필요 
- 더 나아가서는 유저가 많은 환경에서의 개발에 대해서도 공부하고 생각
  - 실제로 구현해본 경험은 없더라도 어떻게 구현하고 어떤 기술과 프레임워크들을 활용하면 좋을지에 대한 고민은 필수적
- Docker와 Kubernetes를 활용하는 이유는 무엇인지, 모니터링 솔루션은 어떤 것들이 있는지
- 이중화(High Availability), MSA와 같은 키워드에 대해서 고민해볼 것
...
## 메모리의 종류와 할당 방식
- 데이터 영역: 전역 변수, 정적(Static) 변수 → 프로그램 실행 시 로딩되는 영역으로, 중요도는 떨어짐
- **힙(Heap) 영역**: 프로그래머가 할당하는 영역 → 런타임의 크기를 결정
- **스택(Stack) 영역**: 지역 변수, 매개 변수 → 컴파일 타임의 크기를 결정

## 메모리가 중요한 이유
- 메모리는 무한하지 않아, 관리가 필요
- 프로그램이 종료되면, 할당된 모든 메모리는 날아감
- 메모리에 값을 읽고 쓰는 행위는 반드시 동시에 일어나선 안 됨